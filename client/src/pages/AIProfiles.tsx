...
export default function AIProfiles() {
  ...
  return (
    <AppLayout>
      <PageHeader
        title="AI Profiles"
        description="Configure how the AI writes your emails."
        actions={
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          <p className="text-muted-foreground col-span-3 text-center">Loading profiles...</p>
        ) : profiles?.length === 0 ? (
          <div className="col-span-3 text-center py-16 bg-card border border-border/50 rounded-xl border-dashed">
            ...
            <h3 className="text-lg font-semibold">No AI Profiles</h3>
            <p className="text-muted-foreground max-w-sm mx-auto mt-2 mb-6">
              Create a profile to teach the AI your tone and value proposition.
            </p>
            ...
            <div key={profile.id} className="bg-card border border-border/50 rounded-xl p-6 shadow-sm hover:shadow-md transition-all">
              ...
              <div className="bg-muted/50 rounded-lg p-3 mb-4">
                <div className="flex items-center gap-2 mb-2 text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                  <MessageSquare className="h-3 w-3" /> System Instructions
                </div>
              ...
              <div className="flex gap-2">
                <Button variant="outline" className="w-full text-sm">Edit Instructions</Button>
              </div>
            </div>
          ))
        )}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>Create AI Profile</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
          <div className="space-y-2">
            <Label htmlFor="name">Profile Name</Label>
            <Input id="name" {...form.register("name")} placeholder="e.g. Friendly Tech Sales" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea 
              ...
              placeholder="You are an expert sales representative. Write short, punchy emails..."
              className="min-h-[150px]"
            />
            <p className="text-xs text-muted-foreground">
              Describe the persona, tone, and constraints for the AI.
            </p>
          </div>
