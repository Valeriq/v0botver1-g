...
Автономный агент для ведения диалогов:
1. Отправляет первое письмо → ждёт ответ
2. Классифицирует ответ:
   - **Вопрос** → генерирует ответ → отправляет (авто)
   - **Заинтересован** → создаёт lead → уведомляет человека
   - **Отказ** → закрывает thread → желает всего хорошего
3. Работает без подтверждения при понятных сценариях
